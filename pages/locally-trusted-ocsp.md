...
## Introduction
In the Public Key Infrastructure (PKI), the Online Certificate Status Protocol (OCSP) is used to determine the status of a public key certificate [FIPS 201-2, NIST IR 7298 (Rev. 2)]. An OCSP Responder can functionally replace Certificate Revocation Lists (CRLs) <!-- Correct?  Need a footnote or definition of CRL, since PIV-Guides plain language guidelines, etc., say to assume no prior knowledge of subject matter. -->. A response from an OCSP Responder <!-- a definition would be helpful. -->, as with a CRL, will tell you whether a public key certificate has been revoked by its Certification Authority (CA) (i.e., the CA [its issuer] that certified it in the first place). **STOPPED HERE** If your organization relies on external, Internet-hosted sources for critical functionality within your local network, including certificate status information, it could make sense <!-- availability of what? -->to create a locally hosted (and locally trusted) copy of critical information on revoked certificates by using a locally trusted OCSP Responder. Hosting this service locally can ensure that clients will not experience the Internet disruptions, outages, or problems that could occur when relying on remotely hosted CRLs or OCSP.
